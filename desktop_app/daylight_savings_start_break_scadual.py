...
class App(customtkinter.CTk):
        ...
        def start_break_sorter():
            ...
            try:
                ...
                if check_valid_date(date) == False:
                    raise ValueError
                self.break_sorter = DLS_start_break_sorter(date, location=globals.LOCATION)
    ...
    def roll_gaming_day(self):
        ...
        self.break_sorter = DLS_start_break_sorter(date, location)
        self.break_sorter.add_hours()
    ...
    def load_state(self):
        ...
        self.break_sorter = DLS_start_break_sorter(state["break sorter"]["date"], state["break sorter"]["location"])
        self.break_sorter.add_hours()
        if len(self.tables) > 0:
            for table in self.tables:
                self.break_sorter.add_table(table)
