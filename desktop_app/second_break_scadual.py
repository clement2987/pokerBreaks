...
from helpers import get_formatted_date, check_valid_date, get_time_nearest_15, get_timestamp, get_date_time_from_timestamp, get_time_from_timestamp, find_time_index, save_state_json, load_state_json, get_gaming_day_base, add_days_to_date, hour_change
import globals
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.sidebar_button_8 = customtkinter.CTkButton(self.sidebar_frame, width=button_width,  text="temp", command=self.browse_files)
    ...
    def log(self, message):
        ...
        customtkinter.CTkLabel(self.tabs.tab("help"), text=message).pack(pady=10)
    ...
    def break_sorter_window(self):
        self.log("break sorter window created")
        ...
        def start_break_sorter():
            self.log("start break sorter button pressed")
            s = entry.get()
            self.log("entry is" + s)
            try:
                day, month, year = map(int, s.split('-'))
                date = f"{year}-{month:02d}-{day:02d}"
                self.log("checking valid date")
                if check_valid_date(date) == False:
                    self.log("value error in valid date")
                    raise ValueError
                self.log("attempting to create break sorter")
                self.break_sorter = Break_sorter(date, location=globals.LOCATION)
                self.log("break sorter created")
                ...
                self.populate_main_display(time())
                self.log("starting time check")
                self.time_check()
                self.log("about to close table")
                cancel()
            except ValueError:
    ...
    def open_table(self):
        ...
        def add_table():
            self.log("add table function working")
            tn = table_number.get()
